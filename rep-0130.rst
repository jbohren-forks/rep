...
build.  Other tools (e.g. rospack) used the ROS_PACKAGE_PATH variable
to determine the location of a package given a package name.
The order of entries in ROS_PACKAGE_PATH allowed overlaying packages,
with similarly named packages, earlier entries would be preferred.
...
overlaying order within a workspace. Instead, catkin crawls the whole
source space for package folders, with the ability to prevent crawling
subfolders by using marker files, ignoring any .rosinstall file. This
functionality is provided within the catkin_pkg package, and the
design choice to crawl all subfolders instead of a list maintained by
rosinstall/rosws was historically made to allow the feature of flat
workspaces with minimal lookup time, a feature that since then has
been dropped. In other words, it is thinkable for catkin to revert
back to crawling a whitelisted set of folders for sources and allow
overlaying instead of crawling a folder.

So with rosws, a folder copied into the workspace iss not part of the
...
can only be achieved instead by chaining whole workspaces.
